...
writeModuleFile mNamespace mIncFile mFile fileWriter cf m = do
  let fs0 = FState Set.empty  [] (mNamespace (m_name m))
      ...
      ifileLines = map T.stripEnd [ T.concat ["// @", "generated from adl module ", formatText (m_name m)]
      ...
      cppfileLines = map T.stripEnd [ T.concat ["// @", "generated from adl module ", formatText (m_name m)] ] ++ fileLines cppfileElements
...
cPrimitiveType P_Json _ = return "nlohmann::json"
...
cPrimitiveDefault P_Json = Just "nullptr"
...
cPrimitiveLiteral P_Json (jv) = template "nlohmann::json::parse($1)" [T.pack (show (JSON.encode jv))]
