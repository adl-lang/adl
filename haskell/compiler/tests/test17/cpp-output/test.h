...
template <class A, class B>
struct Serialisable<ADL::test::Pair<A,B>>
{
    static typename Serialiser<ADL::test::Pair<A,B>>::Ptr serialiser(const SerialiserFlags &);
};

template <class A, class B>
typename Serialiser<ADL::test::Pair<A,B>>::Ptr
Serialisable<ADL::test::Pair<A,B>>::serialiser( const SerialiserFlags &sf )
{
    typedef ADL::test::Pair<A,B> _T;
